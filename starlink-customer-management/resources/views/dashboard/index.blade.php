...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <!-- Total Customers -->
        <div class="overflow-hidden rounded-xl bg-white dark:bg-gray-800 shadow-sm ring-1 ring-gray-900/5 dark:ring-gray-700">
            <div class="p-6">
    ...
    <div class="rounded-xl bg-white dark:bg-gray-800 shadow-sm ring-1 ring-gray-900/5 dark:ring-gray-700 p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <p class="text-2xl font-bold text-gray-900 dark:text-white">Rp {{ number_format($monthlyPayments, 0, ',', '.') }}</p>
            </div>
            <div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <div class="px-6 py-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $payment->customer->nama }}</p>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $payment->periode_bulan }}</p>
                        </div>
        ...
        <div class="rounded-xl bg-white dark:bg-gray-800 shadow-sm ring-1 ring-gray-900/5 dark:ring-gray-700">
            ...
            <div class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($upcomingDue as $customer)
                <div class="px-6 py-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $customer->nama }}</p>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $customer->paket ?? 'N/A' }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-sm font-medium {{ $customer->tanggal_jatuh_tempo->isPast() ? 'text-red-600 dark:text-red-400' : 'text-orange-600 dark:text-orange-400' }}">
                                {{ $customer->tanggal_jatuh_tempo->format('d M Y') }}
                            </p>
                            <p class="text-xs text-gray-500 dark:text-gray-400">
                                {{ $customer->tanggal_jatuh_tempo->diffForHumans() }}
                            </p>
                        </div>
                    </div>
                </div>
                @empty
                <div class="px-6 py-4 text-center text-sm text-gray-500 dark:text-gray-400">
                    Tidak ada jatuh tempo dalam 7 hari ke depan
                </div>
                @endforelse
            </div>
        </div>
