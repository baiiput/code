 ...
 * 🔧 FIX: Since payment type and nominal are now per-KIT (entered in Step 2),
 * Step 3 validation just checks if all KITs have required data
 */
function validateStep3() {
    // 🔧 NEW APPROACH: Validate based on per-KIT data from Step 2
    // Since payment type and nominal are now entered per-KIT in Step 2,
    // Step 3 just needs to ensure Step 2 data is complete
    ...
    if (window.selectedKits && window.selectedKits.length > 0) {
        ...
        // No KITs selected = invalid
        allKitsHavePaymentType = false;
        allKitsHaveValidNominal = false;
    }

    const isValid = allKitsHavePaymentType && allKitsHaveValidNominal;

    console.log('🔍 Step 3 Validation (Per-KIT Mode):', {
        selectedKitsCount: window.selectedKits ? window.selectedKits.length : 0,
        allKitsHavePaymentType: allKitsHavePaymentType,
        allKitsHaveValidNominal: allKitsHaveValidNominal,
        isValid: isValid
    });
...
function setupFormValidation() {
    ...
    // 🔧 FIX: Explicit listeners for Step 3 fields (tipePembayaran & nominal)
    const tipePembayaranInput = document.getElementById('tipePembayaran');
    if (tipePembayaranInput) {
        tipePembayaranInput.addEventListener('change', function() {
            console.log('💳 Payment type changed:', this.value);
            setTimeout(() => {
                updateNavigationButtons();
            }, 100);
        });
        tipePembayaranInput.addEventListener('input', function() {
            console.log('💳 Payment type input:', this.value);
            setTimeout(() => {
                updateNavigationButtons();
            }, 100);
        });
    ...
    if (nominalInputField) {
        nominalInputField.addEventListener('change', function() {
            console.log('💰 Nominal changed:', this.value);
            setTimeout(() => {
                updateNavigationButtons();
            }, 100);
        });
        nominalInputField.addEventListener('input', function() {
            console.log('💰 Nominal input:', this.value);
            setTimeout(() => {
                updateNavigationButtons();
            }, 50);
        });
...
document.getElementById('paymentForm')?.addEventListener('submit', function(e) {
    // 🔧 FIX: ALWAYS prevent default form submission to avoid page reload
    e.preventDefault();
    e.stopPropagation();
