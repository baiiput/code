...
// Simple function to get expiry information
$expiry_info = null;
if (isset($GLOBALS['validated_client']) && !empty($GLOBALS['validated_client'])) {
    try {
        $client = $GLOBALS['validated_client'];
        $expired_date = $client['tanggal_expired'];
        $current_time = time();
        $expire_time = strtotime($expired_date);
        
        // Calculate days remaining
        $days_remaining = floor(($expire_time - $current_time) / 86400);
        
        // Get current subdomain
        $host = $_SERVER['HTTP_HOST'];
        $current_subdomain = str_replace('.octonet.my.id', '', $host);
        $current_subdomain = str_replace('www.', '', $current_subdomain);
        $current_subdomain = strtolower(trim($current_subdomain));
        
        $expiry_info = array(
            'id' => $client['id'],
            'expired_date' => $expired_date,
            'expired_formatted' => date('d M Y', strtotime($expired_date)),
            'expired_time' => date('H:i', strtotime($expired_date)),
            'days_remaining' => $days_remaining,
            'is_expired' => $days_remaining < 0,
            'is_expiring_soon' => $days_remaining >= 0 && $days_remaining <= 7,
            'client_name' => $client['nama'],
            'subdomain' => $client['subdomain'],
            'current_subdomain' => $current_subdomain,
            'email' => $client['email'],
            'phone' => isset($client['no_wa']) ? $client['no_wa'] : '-',
...
<style>
#brand {
    font-size: 0 !important; /* Sembunyikan teks asli "MIKHMON" */
    position: relative;
}

#brand::after {
    content: "Hotspot Management"; /* Nama brand baru Anda */
...
<?php 
  ...
  <?php if ($expiry_info): ?>
  <!-- Enhanced Expiry Information for Admin Panel -->
  <div style="margin: 10px 8px; padding: 10px; background: <?php 
    if ($expiry_info['is_expired']) {
        echo 'linear-gradient(135deg, #dc3545, #c82333)';
    } elseif ($expiry_info['is_expiring_soon']) {
        echo 'linear-gradient(135deg, #e67e22, #d35400)';
    } else {
        echo 'linear-gradient(135deg, #27ae60, #229954)';
    }
  ?>; color: white; border-radius: 8px; font-size: 11px; text-align: left; box-shadow: 0 2px 6px rgba(0,0,0,0.2);">
    <div style="font-weight: bold; margin-bottom: 3px; display: flex; align-items: center;">
      ...
      <?= htmlspecialchars($expiry_info['client_name']) ?>
    </div>
    <div style="margin-bottom: 2px; opacity: 0.9;">📅 Expired: <?= $expiry_info['expired_formatted'] ?></div>
    <div style="margin-bottom: 6px; font-weight: 600;">
      <?php if ($expiry_info['is_expired']): ?>
        ⏰ Expired <?= abs($expiry_info['days_remaining']) ?> hari lalu
      <?php elseif ($expiry_info['days_remaining'] == 0): ?>
        ⏰ Expired hari ini!
      <?php else: ?>
        ⏰ <?= $expiry_info['days_remaining'] ?> hari lagi
      <?php endif; ?>
    </div>
    <div style="margin-top: 8px;">
      <a href="https://payment.octonet.my.id/renewal.php?subdomain=<?= urlencode($expiry_info['current_subdomain']) ?>" 
         style="display: inline-block; background: rgba(255,255,255,0.2); color: white; padding: 4px 8px; border-radius: 12px; text-decoration: none; font-size: 10px; font-weight: 600; transition: all 0.3s ease; border: 1px solid rgba(255,255,255,0.3);" 
         target="_blank"
         onmouseover="this.style.background='rgba(255,255,255,0.3)'; this.style.transform='translateY(-1px)'; this.style.boxShadow='0 2px 4px rgba(0,0,0,0.2)'"
...
<div id="sidenav" class="sidenav">
  ...
  <?php if ($expiry_info): ?>
  <!-- Enhanced Expiry Information for Main Panel -->
  <div style="margin: 10px 8px; padding: 10px; background: <?php 
    if ($expiry_info['is_expired']) {
        echo 'linear-gradient(135deg, #dc3545, #c82333)';
    } elseif ($expiry_info['is_expiring_soon']) {
